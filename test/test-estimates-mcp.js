...
class TestClient {
    ...
    async run() {
        ...
        // Show estimate tools
        const estimateTools = toolsRes.result.tools.filter(t => t.name.includes('estimate'));
        console.log('\nEstimate Tools:');
        estimateTools.forEach(tool => {
            console.log(`  - ${tool.name}: ${tool.description}`);
        });

        // 3. Test get_estimates
        console.log('\nTesting get_estimates tool...');
        const estimatesRes = await this.request('tools/call', {
            name: 'get_estimates',
            arguments: { limit: 5 }
        }, 3);

        console.log('✅ Estimates list result received');
        console.log(JSON.stringify(estimatesRes.result, null, 2));
