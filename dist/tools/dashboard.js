...
async function handleDashboardTool(toolName, args, api) {
    switch (toolName) {
        case 'get_dashboard_stats': {
            ...
            const recentInvoices = (dashboard.recent_invoices || []).map((inv) => `- #${inv.invoice_number || inv.id}: ${inv.total ? formatMoney(inv.total) : 'C$ 0.00'} (${inv.status}) - ${inv.customer?.name || 'Unknown Customer'}`).join('\n');
